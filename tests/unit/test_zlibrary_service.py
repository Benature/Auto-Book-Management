...
def test_find_best_match(mock_zlib_client, zlibrary_service):
    ...
    # 测试找到精确匹配
    best_match = service.find_best_match(title='Test Book', author='Test Author')
    assert best_match['id'] == '123456'

    # 测试找到部分匹配
    best_match = service.find_best_match(title='Test Book Second', author='Test Author')
    assert best_match['id'] == '789012'

    # 测试未找到匹配
    mock_client_instance.search.return_value = []
    best_match = service.find_best_match(title='Nonexistent Book', author='Unknown Author')
    assert best_match is None

@patch('services.zlibrary_service.AsyncZlib')
@patch('services.zlibrary_service.open', new_callable=mock_open)
@patch('services.zlibrary_service.os.path.exists')
def test_download_book(mock_exists, mock_file_open, mock_zlib_client, zlibrary_service):
    """测试下载书籍功能"""
    ...
    expected_path = os.path.join(download_dir,
...
def test_search_and_download(mock_zlib_client, zlibrary_service):
    ...
    # 模拟文件不存在
    with patch('services.zlibrary_service.os.path.exists',
               return_value=False):
        # 模拟文件写入
        with patch('services.zlibrary_service.open',
            ...
            assert result['file_path'] == os.path.join(download_dir,
                             'Test Book - Test Author.epub')
