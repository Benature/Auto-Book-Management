...
class TaskScheduler:
    ...
    def schedule_task(self, name: str, schedule_type: str, **schedule_kwargs) -> bool:
        """
        设置任务调度
        ...
        if schedule_type == 'daily':
            ...
            task['schedule'] = schedule.every().day.at(schedule_kwargs['at']).do(task_wrapper)
            ...
            task['schedule'] = day_mapping[day].at(schedule_kwargs['at']).do(task_wrapper)
            ...
            if 'hours' in schedule_kwargs and schedule_kwargs['hours'] > 0:
                task['schedule'] = schedule.every(schedule_kwargs['hours']).hours.do(task_wrapper)
                next_run = datetime.now() + timedelta(hours=schedule_kwargs['hours'])
                interval_set = True
                
            elif 'minutes' in schedule_kwargs and schedule_kwargs['minutes'] > 0:
                task['schedule'] = schedule.every(schedule_kwargs['minutes']).minutes.do(task_wrapper)
                next_run = datetime.now() + timedelta(minutes=schedule_kwargs['minutes'])
                interval_set = True
                
            elif 'seconds' in schedule_kwargs and schedule_kwargs['seconds'] > 0:
                task['schedule'] = schedule.every(schedule_kwargs['seconds']).seconds.do(task_wrapper)
                next_run = datetime.now() + timedelta(seconds=schedule_kwargs['seconds'])
                interval_set = True
    ...
    def _calculate_next_run_time(self, schedule_type: str, schedule_kwargs: Dict[str, Any]) -> datetime:
        """
        计算下次执行时间
