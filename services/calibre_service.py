...
class CalibreService:
    ...
    def __init__(self, server_url: str, username: str, password: str, match_threshold: float = 0.6):
        """
        初始化 Calibre 服务
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = self.session.get(f"{self.server_url}/ajax/library-info", timeout=10)
    ...
    def search_book(self, title: str, author: Optional[str] = None, isbn: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        搜索书籍
        ...
        try:
            ...
            if data:
                ...
                book_info = {
                    ...
                    'cover_url': f"{self.server_url}/get/cover/{book_id}/default",
    ...
    def find_best_match(self, title: str, author: Optional[str] = None, isbn: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        找到最佳匹配的书籍
        ...
        for result in results:
            ...
            # 标题匹配度（简单字符串相似度）
            title_similarity = self._calculate_similarity(title.lower(), result['title'].lower())
            score += title_similarity * 0.7  # 标题权重 70%
            
            # 作者匹配度
            if author and result.get('author'):
                author_similarity = self._calculate_similarity(author.lower(), result['author'].lower())
        ...
        if best_match and best_score >= self.match_threshold:
            self.logger.info(f"找到最佳匹配: {best_match['title']}, 分数: {best_score:.2f}")
            return best_match
        else:
            self.logger.info(f"未找到满足阈值的匹配，最佳分数: {best_score:.2f}, 阈值: {self.match_threshold}")
            return None
    ...
    def upload_book(self, file_path: str, metadata: Optional[Dict[str, Any]] = None) -> Optional[int]:
        """
        上传书籍
        ...
        try:
            ...
            files = {'file': (file_path.name, open(file_path, 'rb'), f'application/{file_path.suffix[1:]}')}
            ...
            self.logger.error(f"上传书籍失败: {file_path.name if file_path else 'Unknown'}, 错误: {str(e)}")
            return None
        finally:
            # 确保文件已关闭
